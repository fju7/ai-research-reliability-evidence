# WHU Requirements Traceability v2.1

**Status:** narrow closure-review candidate
...
| `REQ-016` Surveillance/correction | Generic contracts/tests | `CONTRACT_IMPLEMENTED` | Frozen maintenance rule/burden ceiling and prospective pass/stop evidence | F |
...
| `REQ-019` Independent evaluation | Research apparatus; no qualified product reference | `GAP` | Anchor/role necessity map; bounded human decision; role separation; protected reference; approved analysis | A-B, E |
| `REQ-020` Full economics | Cost/review fields; isolated run costs | `GAP` | Validation affordability distinct from repeatable product TCO | A-B, H |
...
| `REQ-024` Explicit decision | No product-decision object yet | `GAP` | Human-reference decision, bounded-probe disposition, and final product outcome | A-B, I |
...
| `REQ-027` Program exhaustion | Development cost policy only | `GAP` | Costing before envelope, protected Phase B reserve, and enforced exhaustion record | A-I |
...
| Anchor/role necessity map | target property, matching anchor, human necessity, role combination, low/base/high cost | before envelope selection | target, anchor, role, or cost-basis change |
...
| Instrument-lineage record | authorship/model/source provenance and material independent defect challenge | before confirmatory use | taxonomy, planted set, rubric, reader instrument, or synthesis-lineage change |
| Economics charter | cost worksheet, Phase B reserve, selected envelope, validation/product denominators, full TCO | costing before selection; reserve before Phase A spend | price, allocation, volume, horizon, or product-market change |
...
| Maintenance rule | events, materiality, detection/propagation/review/reconstruction, burden ceiling | before Phase F evaluated outcomes | route, event, rubric, reference, burden, or analysis change |
...
calibration, qualified independent reference where truly necessary, global warrant, comparator
performance, statistical identification under shared dependencies, hard-case coverage, validation
affordability, and repeatable product economics.
