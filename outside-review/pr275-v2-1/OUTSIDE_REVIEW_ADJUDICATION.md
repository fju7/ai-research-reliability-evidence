# Adjudication of WHU PRD/Plan Outside Reviews through v2.1
...
## 0. PR #274 v2 closure findings N-1–N-6

The same reviewer returned `READY_WITH_MATERIAL_FIXES`. V2.1 treats the six findings as local closure
work and does not reopen the architecture.

### N-1 — Cost validation before envelope selection and protect Phase B

**Disposition:** `ACCEPT_IN_SUBSTANCE_WITH_DIFFERENT_FIX`.

**Rationale:** selecting an envelope without a costed validation design can exhaust funds on Phase A
apparatus. The reviewer's published rates are useful possible inputs, but its role hours and six-
separate-person reading are not established planning facts.

**Exact v2.1 change:** PRD §§4.2 and 7 and Plan Phase A require a Validation Anchor / Role Necessity
Map and a low/base/high costing worksheet for every validation function before Fred selects any
envelope. Before any Phase A spending, a named dollar/percentage Phase B reserve must freeze. Failure
to preserve it returns `VALIDATION_NOT_AFFORDABLE_AT_THIS_SCALE`.

**Humans immediately required?** No. The map first tests matching formal, executable,
authoritative-structured, protected-reference, outcome, and professional-method anchors.

**Deferred human decision:** only a target property that cannot be credibly validated by those
anchors enters the explicit human-reference decision point.

### N-2 — Validation affordability is not product economics

**Disposition:** `ACCEPT`.

**Rationale:** inability to afford a study within this project's envelope does not show that WHU's
repeatable process is uneconomic.

**Exact v2.1 change:** separate `VALIDATION_NOT_AFFORDABLE_AT_THIS_SCALE` from
`PRODUCT_ECONOMICS_IMPLAUSIBLE` in the PRD, Plan, traceability, exhaustion, and not-ship paths. Every
cost stop names its denominator.

**Humans immediately required?** No.

**Deferred human decision:** human reference cost enters validation affordability first and product
economics only when it is part of the repeatable product process.

### N-3 — Favorable Phase B result overclaims

**Disposition:** `ACCEPT`.

**Rationale:** a frozen-corpus probe cannot test acquisition/evidence-selection adequacy,
open-world completeness, cross-domain reliability, or production readiness.

**Exact v2.1 change:** rename the result `BOUNDED_SYNTHESIS_CORE_PROMISING`, state its exact tested
scope, and permit only Phase C. The excluded properties are explicit in PRD §5 and Plan Phase B.

**Humans immediately required?** No.

**Deferred human decision:** any later human necessity is target-property-specific and does not
expand the Phase B inference.

### N-4 — Role-separation equivalence escape hatch

**Disposition:** `ACCEPT`.

**Rationale:** an unbounded equivalence claim would be most tempting under the very cost/recruitment
pressure independence is meant to withstand.

**Exact v2.1 change:** only the named independent protocol/statistical approver may accept an
equivalence design; the argument freezes in the independence map before evaluated output or
recruitment outcomes. Cost, schedule, recruitment difficulty/failure, convenience, and model-agent
agreement cannot establish equivalence.

**Humans immediately required?** No. Strict separation is required only after the anchor map shows
that the target genuinely requires qualified human judgment and no valid equivalence exists.

**Deferred human decision:** record `QUALIFIED_HUMAN_REFERENCE_REQUIRED`, then present Fred with the
bounded need, qualifications, design, costs, disagreement treatment, and consequence of declining.

### N-5 — Phase F maintenance lacks freezes and failure criteria

**Disposition:** `ACCEPT`.

**Rationale:** maintenance cannot be falsifiable when its rule and burden ceiling are not frozen and
the phase has no explicit adverse outcome.

**Exact v2.1 change:** Plan Phase F now has entry criteria, frozen maintenance/burden rules and
fixtures before outcomes, explicit pass criteria, explicit stop criteria, and retry discipline.

**Humans immediately required?** No; Phase F is not authorized or executed.

**Deferred human decision:** reference refresh may later trigger the human-reference decision if
the target property cannot be maintained through other defensible anchors.

### N-6 — System and instrument share blind spots

**Disposition:** `ACCEPT_IN_SUBSTANCE_WITH_DIFFERENT_FIX`.

**Rationale:** independent approval and statistical modeling cannot recover an omitted defect class.
Requiring a large human panel to author every instrument is unnecessary, but the taxonomy and
planted set need a material challenge outside the synthesis-design lineage.

**Exact v2.1 change:** record authorship/model/source lineage for synthesis, taxonomy, planted set,
rubric, and reader instrument. Before confirmatory use, require a material independent contribution
or challenge from published taxonomies, external benchmarks/incidents, outside-review findings,
professional-method evidence, another demonstrably independent source, or later qualified humans.
A different model family alone is insufficient.

**Humans immediately required?** No.

**Deferred human decision:** if adequate independent defect generation cannot be obtained without
qualified humans, surface that exact need at the human-reference decision point.

## 0.1 Closure conclusion

V2.1 accepts all six defects while rejecting unsupported headcount/hour assumptions and categorical
ethics conclusions. It preserves reader/reference separation and adds an actual pre-recruitment
human-reference decision. Once the same reviewer confirms N-1–N-6 closure, further architecture
redesign stops unless new Phase A/B evidence requires it.
