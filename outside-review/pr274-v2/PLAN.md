# WHU Gated Validation Plan v2

**Status:** outside-review candidate; no execution authorization
**Base:** `e56e2d18cddfd2814711f75be36409cbbc4474d1`
**Governance:** PR #273 remains frozen and unmerged; engineering remains paused
...
- Fred selects and records one budget/calendar/retry envelope from `PRODUCT_REQUIREMENTS.md` §7.
...
| B | Run cheap Epistemic Feasibility Probe | one of four probe dispositions |
...
Only `EPISTEMIC_CORE_PROMISING` at Phase B permits Phase C. Phases C-I preserve the strongest v1
...
4. **Independence map:** named incompatible roles, qualification/conflict criteria, blinding, access
   controls, recruitment feasibility, and fallback disposition.
...
6. **Defect/case calibration protocol:** naturalistic versus planted sets, pilot, difficulty and
   consequentiality strata, and freeze rules.
7. **Economics charter:** selected program envelope; total-cost categories; labor rates;
   amortization horizon; invalidation events; absolute and relative decision rules.
...
- Fred owns product-market scope and selects the program envelope; he is not the biomedical
  reference oracle.
...
- Qualified domain people approve the substantive reference and rubric scope.
- The plan records which roles are external, their expected price/range, and whether recruitment is
  feasible. No recruitment occurs under this planning PR.

### Mandatory freeze points

- The comparator, independence map, rubric definitions, sampling plan, probe question/corpus, and
  program envelope freeze before any probe output is generated.
...
- **Pass:** all eight records exist, have named independent approvals, pass a leakage/access review,
  and fit the selected envelope.
- **Stop—`QUALIFIED_REFERENCE_UNAVAILABLE`:** qualified independent reference/adjudication roles
  cannot be supplied within the envelope.
...
- **Stop—`ECONOMICALLY_IMPLAUSIBLE`:** credible instrument/reference estimates already exceed the
  frozen economics.
...
Phase A passed; a separate execution authorization exists; protected reference and reader roles are
staffed; inventory hashes and access are verified; no additional search or production consumer is
enabled.
...
- `EPISTEMIC_CORE_PROMISING`
- `EPISTEMIC_CORE_NOT_YET_DEMONSTRATED`
- `QUALIFIED_REFERENCE_UNAVAILABLE`
- `ECONOMICALLY_IMPLAUSIBLE`

Only the first opens Phase C. Reference unavailability is a result, not indefinite `NOT_ASSESSED`.
...
Before opening the v2 planning PR, verify:
...
- v1 history and PR #273 remain unchanged.

## 14. Immediate governance action

Freeze this v2 package after focused internal review and CI. Send it to the same independent
reviewer before merge. Do not resume engineering unless that review returns
`READY_WITH_MATERIAL_FIXES` or better and the owner separately authorizes merge/resumption. Do not
run the probe, recruit participants, contact the reviewer, publish, or change production state as
part of this planning PR.
